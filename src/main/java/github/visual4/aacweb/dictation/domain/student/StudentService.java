...
public class StudentService {
	...
	public User regiserStudent(Long teacherSeq, TypeMap studentInfo) {
		...
		/*
		 * List<License> licenses = licenseService.findsBy(License.Column.receiver_ref,
		 * teacher.getSeq());
		 ...
		 * String.format("%d students of %d licenses", students.size(),
		 * licenses.size()));
		 * }
		 */
		...
		student.setPass(encodedPassword);
		student.setPassEncoded(YesNo.Y);
	...
	public TypeMap login(String studentId, String password, boolean autoLogin) {
		...
		if (!autoLogin && !matchesPassword(password, student.getPass())) {
	...
	public User update(Long teacherSeq, User form) {
		...
		String rawPass = form.getPass();
		String encodedPass = passwordEncoder.encode(rawPass);
		student.setPass(encodedPass);
		student.setPassEncoded(YesNo.Y);
		studentDao.updateStudent(student);
		student.setPass(rawPass);
		return student;
	}
	...
	private boolean matchesPassword(String rawPass, String encodedPass) {
		if (rawPass == null || encodedPass == null) {
			return false;
		}
		if (!encodedPass.startsWith("$2a$")
